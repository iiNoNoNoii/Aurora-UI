...
const SCHEMA = [
  ...
  },
  {
    ...
    schema: [
      ...
      numberRow('blur', 0, 60, 1),
      numberRow('opacity', 0, 1, 0.05),
      numberRow('saturate', 1, 3, 0.05),
      numberRow('glow', 0, 2, 0.05),
      { name: 'radius', selector: { number: { min: -1, max: 60, step: 1, mode: 'box' } } },
      boolRow('border'),
      boolRow('adaptive_text'),
    ],
  },
  {
    name: 'background',
...
export class AuroraBackgroundEditor extends LitElement {
  ...
  private get formData(): Record<string, unknown> {
    ...
    return {
      ...
      glass: { ...resolved.glass, preset_entity: resolved.glass.preset_entity ?? '' },
      background: {
        transparent_lovelace: resolved.background.transparent_lovelace,
        header: resolved.background.header,
        ambient_variables: resolved.background.ambient_variables,
...
Please configure the card in YAML – see the repository README.
  ...
  private onValueChanged(event: CustomEvent): void {
    ...
    const next: AuroraBackgroundConfigInput = {
      ...
      glass: value.glass as AuroraBackgroundConfigInput['glass'],
      background: {
